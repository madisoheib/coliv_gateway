...
@section('title', 'New Supervisor Program')

@section('content')
    <div class="mb-8 md:flex md:items-center md:justify-between">
        <div class="min-w-0 flex-1">
            <h2 class="text-2xl font-bold text-gray-900">New Supervisor Program</h2>
            <p class="mt-1 text-sm text-gray-500">Create a new supervised process configuration</p>
        </div>
        <div class="mt-4 md:mt-0">
            <a href="{{ route('supervisor.index') }}" class="inline-flex items-center rounded-md bg-white px-3 py-2 text-sm font-semibold text-gray-900 shadow-sm ring-1 ring-gray-300 ring-inset hover:bg-gray-50">
                &larr; Back
            </a>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div class="lg:col-span-2">
            <form method="POST" action="{{ route('supervisor.programs.store') }}" id="program-form" class="bg-white shadow sm:rounded-lg">
                @csrf
                <div class="px-4 py-5 sm:p-6 space-y-6">
                    ...
                    <div>
                        ...
                        <select name="container" id="container" required
                            ...
                            @foreach($containers as $key => $info)
                                <option value="{{ $key }}" {{ old('container') === $key ? 'selected' : '' }}>{{ $info['label'] }} ({{ $key }})</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div>
                        ...
                        <input type="text" name="program_name" id="program_name" required maxlength="64"
                               pattern="[a-zA-Z0-9_-]+" value="{{ old('program_name') }}"
                               class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm"
                               placeholder="my-worker">
                        <p class="mt-1 text-xs text-gray-500">Letters, numbers, hyphens, underscores only</p>
                    </div>
                    ...
                    <div>
                        ...
                        <input type="text" name="command" id="command" required maxlength="500"
                               value="{{ old('command') }}"
                               class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm"
                               placeholder="php /var/www/artisan queue:work --tries=3">
                    </div>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                        ...
                        <div>
                            ...
                            <input type="number" name="stdout_logfile_backups" id="stdout_logfile_backups" min="0" max="50"
                                   value="{{ old('stdout_logfile_backups', 5) }}"
                                   class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="text" name="stdout_logfile" id="stdout_logfile" maxlength="255"
                                   value="{{ old('stdout_logfile', '/var/log/supervisor/%(program_name)s.log') }}"
                                   class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                        </div>
                        <div>
                            ...
                            <input type="text" name="stdout_logfile_maxbytes" id="stdout_logfile_maxbytes" maxlength="20"
                                   value="{{ old('stdout_logfile_maxbytes', '10MB') }}"
                                   class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm"
                                   placeholder="10MB">
                        </div>
                    </div>
                            ...
                            <label class="relative flex items-start">
                                <div class="flex h-6 items-center">
                                    ...
                                    <input type="checkbox" name="{{ $field }}" value="1"
                                           {{ old($field, in_array($field, ['autostart', 'autorestart', 'redirect_stderr']) ? '1' : '0') == '1' ? 'checked' : '' }}
